...
#CHECK IF THE INPUT IS PARCELS AND DRONES (just titles)
    #if rf.checkTitles(fileName1, fileName2)==True:
        #do the match!!
    ...
    for a in range(len(listP)):
        ...
        while h:
            ...
            and listP[a][5] <= listD[b][5]:
                #zone and distance and autonomy and weight^^^
                ...
                if timeD.addTimeAsString(time, listP[a][6])[1] <= timeD.convertTime(time):
                    ...
                    writeTB.append([timeD.returnNewDate(time)[0], timeD.returnNewDate(time)[1], listP[a][0], listD[b][0]])
